...
When test-first ordering is not the goal, **[`/testing`](./skills/engineering/testing/SKILL.md)** keeps the same focus on public interfaces, agreed seams, vertical slices, and durable tests while letting the agent choose when tests are written within each slice.
...
- **[implement](./skills/engineering/implement/SKILL.md)** — Build the work described by a spec or set of tickets, driving `/testing` at pre-agreed seams and closing out with `/code-review` before committing.
...
- **[testing](./skills/engineering/testing/SKILL.md)** — General testing guidance for durable automated and integration tests through public interfaces, without prescribing test-first ordering.
